...
- Build: ignore `Nuitka` and `pyinstaller` distribution, always use cx_Freeze.
- Tests: pytest. `uv run pytest` (runs all; real-model tests skip when MNN models in `data/models/mnn`). Useful variants:
